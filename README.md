...
**River Plate** is an unofficial fan tribute. It uses a restrained black, white, and red dark palette, places a maintainer-supplied real Monumental photograph inside the title/tab bar, and adds a compact circular embroidered-crest medallion beside CPU/RAM. Drag the subtle handle on the banner's lower edge to make it taller or shorter; double-click the handle to restore its 72 px default. The chosen height is saved locally. The decoration loads only while the theme is selected and never covers terminal content. Its internal identifier remains `912` so existing preferences continue to work.
